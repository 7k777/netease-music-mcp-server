...
def aes_encrypt(text, key):
    ...
    p = padder.update(text.encode()) + padder.finalize()
    e = Cipher(algorithms.AES(key.encode()), modes.CBC(iv)).encryptor()
    return base64.b64encode(e.update(p) + e.finalize()).decode()
...
def enc_params(params):
    ...
    return {"params": aes_encrypt(aes_encrypt(t, NONCE), k), "encSecKey": rsa_encrypt(k)}

def weapi(ep, data):
    data["csrf_token"] = ""
    body = urllib.parse.urlencode(enc_params(data)).encode()
...
def do_call(name, args):
    if name == "search_songs":
        kw = args.get("keyword",""); lim = min(int(args.get("limit",20)),50)
        r = weapi("/search/get",{"s":kw,"type":1,"limit":lim,"offset":0})
        t = f'搜索 "{kw}" 结果：'
        for i,s in enumerate(r.get("result",{}).get("songs",[])[:lim],1):
        ...
        ss = r.get("songs",[])
        if not ss: return {"content":[{"type":"text","text":"未找到"}],"isError":True}
        s = ss[0]
        ...
        if not lyric: return {"content":[{"type":"text","text":"暂无歌词"}]}
        pl = []
        for line in lyric.split("\n"):
            c = line
            while "[" in c and "]" in c: c = c[c.find("]")+1:].strip()
            if c: pl.append(c)
        ...
        kw = args.get("keyword",""); lim = min(int(args.get("limit",10)),30)
        ...
        if not as_: return {"content":[{"type":"text","text":f'未找到 "{kw}"'}],"isError":True}
        a = as_[0]; t = f"歌手：{a.get('name')}\nID：{a.get('id')}"
        h = weapi("/artist/top/song",{"id":a.get("id"),"limit":10,"offset":0})
        if h.get("code")==200:
            hs = h.get("songs",[])
            if hs:
                t += "\n\n热门歌曲："
                for i,s in enumerate(hs,1): t += f"\n{i}. {s.get('name')} [ID:{s.get('id')}]"
        return {"content":[{"type":"text","text":t}]}
...
class H(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        except: return self._j({"jsonrpc":"2.0","error":{"code":-32700}},400)
        m = body.get("method",""); i = body.get("id"); p = body.get("params",{}) or {}
        if m == "initialize":
            ...
            try: return self._j({"jsonrpc":"2.0","id":i,"result":do_call(p.get("name",""),p.get("arguments",{}) or {})})
            except Exception as e: return self._j({"jsonrpc":"2.0","id":i,"error":{"code":-32000,"message":str(e)}})
        else: return self._j({"jsonrpc":"2.0","id":i,"error":{"code":-32601}})
    def log_message(self,f,*a): print(f"[MCP] {f%a}")
